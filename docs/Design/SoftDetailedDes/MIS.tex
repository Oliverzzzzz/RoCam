...
\begin{table}[h]
  ...
  \begin{tabular}{p{3cm} p{10cm}}
    ...
    JSON                  & JavaScript Object Notation. A lightweight data-interchange format used for API request and response payloads.                                                        \\
...
\begin{itemize}
  \item recordings: list[\texttt{RecordingInfo}] (cached list of recordings most
        recently fetched from the backend)
  \item selected\_recording\_id: str $|$ null (currently selected recording in the UI;
        null if none selected)
  \item last\_refresh\_time\_ms: int $|$ null (timestamp of last successful refresh;
...
\begin{enumerate}
  \item What went well while writing this deliverable? \\[2mm]
        What went well was that the requirements gave us a clear checklist for the Module Guide and the MIS. We could map each feature (arming/disarming, manual control, tracking, recording, downloading) to a small set of responsible modules, and then confirm the mapping again in the traceability tables. This made the document feel structured instead of random, because every section was written to support a requirement. \\[2mm]
        Another thing that went well was using the “design for change” idea. By writing down anticipated changes (vision model updates and UI updates), it became easier to decide what each module should hide. For example, the CV Process Module hides model details and video input handling, while the UI modules hide page layout and user interaction. This helped us keep the interfaces stable and avoid spreading the same decision across multiple modules. \\[2mm]
        The prototypes also helped a lot. Because performance requirements are strict (real-time video, 1080p 60fps, continuous output), we could not rely only on a “paper design.” Running small experiments with the Nvidia pipeline and the Jetson hardware gave us confidence that the chosen module boundaries are realistic. These experiments also made it easier to write the “Services” of each module in simple terms, because we understood what each module can reliably provide and what information should stay hidden behind the interface. \\[2mm]
        Finally, peer review feedback was useful for improving clarity. It pushed us to write more explicit design decisions (for example, why we isolate worker processes and why the API Gateway is the single control surface). Overall, the deliverable went well because the requirements, traceability, and prototypes worked together: the requirements told us what to build, traceability confirmed coverage, and prototypes validated feasibility.

  \item What pain points did you experience during this deliverable, and how did you
        resolve them? \\[2mm]
        Video Stream Abstraction Module (made by Nvidia) was challenging because it
        is extremely complex. Making it more complicated, we want to take advantage
        of the hardware as much as possible to achieve our performance goals. A lot
        of prototype experiments had to be run to make sure the module
        decomposition in the MIS is actually feasable and is performant enough. On
        the bright side, all the prototype code helped us understand the access
        functions and semantics of each module better, that is what went well for
        this deliverable.
  \item Which of your design decisions stemmed from speaking to your client(s) or a
        ...
        A lot of the design decisions were made from the constraints set by the
        Video Stream Abstraction Module. One of the design decision is to spawn
        seperate threads for video processing and talk to them over IPC channels.
        This is made because the video processing logic (made by Nvidia) tend to
        crash and we need to isolate the fault to keep the citical parts of the
        system stable. There are no design decisions stemmed from speaking to our
        clients.
  \item While creating the design doc, what parts of your other documents (e.g.
        requirements, hazard analysis, etc), it any, needed to be changed, and why? \\[2mm]
        No other documents need to be changed, MIS implements all the requirements
        perfectly.
  \item What are the limitations of your solution? Put another way, given unlimited
        ...
        In the current implementation, when the user tries to download a recording,
        no new file is created on the backend, the requested recording is rendered
        on the fly and sent to the browser. This simplifies implementation because
        the entire render video \& download logic can be done in one HTTP request
        and there is no presistant states or temporary files to manage.\\[2mm]
        The problem is, because the video is generated on the fly, we don't know the
        size of the video file ahead of the time, which means the browser won't be
        able to show the download progress. This is bad for user exprience as the
        user will have no idea how long will it take.\\[2mm]
        If we have unlimited resources, we can implement a custom download logic in
        the frontend that shows approximate progress based on the number of frames
        rendered. We can even pre-render all the recordings when the backend has
  ...
  \item Give a brief overview of other design solutions you considered. What are the
        ...
        (LO\_Explores)
        \begin{itemize}
          \item Our initial design combined all video processing logic with all the critical
                logic (e.g. gimbal control) into one process. This created instability, as
                discussed in the response to question 3. We had to move them into separate
                processes. We even seperated the computer vision logic from the display logic
                into seperate processes, this is because we want to keep outputing video even
                if the computer vision logic crashes.
          \item We are using a simple custom database to store all the recordings. It leverages
                the file system structure to index the data. A full-blown database was
                considered but discarded due to the heavy resource utilization.
        \end{itemize}
\end{enumerate}
