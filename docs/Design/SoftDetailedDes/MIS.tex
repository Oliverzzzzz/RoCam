...
\begin{center}
  \begin{tabular}{p{4cm} p{3cm} p{8cm}}
    ...
    unix\_timestamp\_ms         & int                               & System time in milliseconds                                  \\
        ...
        \begin{itemize}
          ...
          \item For each frame in the recorded video:
                \begin{itemize}
                  ...
                  \item Scale the frame by \texttt{osd\_data.scale}.
                  \item Render the text from \texttt{osd\_data} over the frame.
                  \item Encode the frame in a compressed video format and save to \texttt{output\_video\_path}.
                \end{itemize}
        \end{itemize}
  \item exception: None
\end{itemize}
...
\begin{itemize}
  \item transition:
        \begin{itemize}
          \item Create a temporary output stream and save the path to \texttt{output\_video\_pipe}.
...
\]
If \(recId\neq\bot\), disarm is either disallowed or must first stop recording (your implementation chooses the policy; keep interface consistent).
