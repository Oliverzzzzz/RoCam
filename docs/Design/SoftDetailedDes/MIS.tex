...
Let the abstract state of this module be:
\[
...
\]
\[
I_2:\ \ recId \neq \bot \Rightarrow armed = \texttt{true} \ \lor \ armed = \texttt{false}
\]
% (I2 is intentionally weak; you can replace with something stronger if desired.)
...
\[
\{\, true \,\}\ \ disarm\ \ \{\, armed' = \texttt{false} \land cv' = cv \land osd' = osd \land recId' = recId \,\}
\]
...
\noindent\underline{\texttt{status}} (pure observation):
\[
status(S) = SystemStatus(\ldots,\ tracking\_state := trackingState(armed, cv),\ \ldots)
\]

\noindent\underline{\texttt{on\_cv\_data}} (event handler):
\[
\{\, true \,\}\ \ on\_cv\_data(d)\ \ \{\, cv' = d \land osd' = createOSD(d) \,\}
\]
...
\begin{enumerate}
  ...
  \item Give a brief overview of other design solutions you considered. What are the
        ...
        (LO\_Explores)
        \begin{itemize}
          \item Our initial design combined all video processing logic with all critical logic (e.g., gimbal control) into one process. This created instability, as discussed in the response to question 3. We had to move them into separate processes. We even separated the computer vision logic from the display logic into separate processes; for example, we wanted to keep outputting video even if the computer vision logic crashes.
          \item We are using a simple custom database to store all the recordings. It leverages the file system structure to index the data (e.g., one folder per recording with metadata and logs). A full-blown database was considered but discarded due to heavy resource utilization and operational overhead on the target hardware.
        \end{itemize}
\end{enumerate}
