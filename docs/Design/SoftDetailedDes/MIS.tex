...
\begin{enumerate}
  ...
  \item While creating the design doc, what parts of your other documents (e.g.
    ...
    \item What are the limitations of your solution? Put another way, given unlimited
        ...
        With unlimited resources, we would improve this by separating ``render'' from ``download'':
        (1) implement an asynchronous job queue that pre-renders recordings into stored MP4 artifacts
        (or renders in the background when the Jetson is idle), (2) provide progress reporting based
        on frames processed / total frames with estimated time remaining, (3) serve downloads as file
        transfers with known size to enable accurate progress bars and resume/range requests, and (4)
        add caching and concurrency controls so repeated requests do not trigger repeated rendering.
